...
(async () => {
  if (!workerData || !workerData.users || !workerData.messageTemplateParams)
    return;
  ...
  for (const user of users) {
    ...
    if (!/^\d{9}$/.test(mobile)) {
      ...
      failedMessages.push({
        user: user.mobile,
        error: 'Invalid UAE number format',
      });
      continue;
    }
    ...
    try {
      ...
      console.log(
        `Failed to send message to: ${user.mobile} | Error: ${error.message}`,
      );
