    ...
    if (process.env.NODE_ENV === 'production') {
      try {
        ...
        const unsubscribedUser =
          await this.prismaService.unsubscribedUser.findMany({
            select: { phone: true },
          });
  ...
  async SendInspectionDetails(
    ...
    try {
      ...
      try {
        ...
        const googleLocationLink = aucitonData.location.lat
          ? `🗺️ Google map: https://www.google.com/maps?q=${aucitonData.location.lat},${aucitonData.location.lng}`
          : ' ';
        const messageTemplateParams = {
