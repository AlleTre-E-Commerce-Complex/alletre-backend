...
export class EmailBatchService {
  ...
  async sendBulkEmails(updatedAuction:any) {
    ...
    const html = `
                ...
                <img src="${updatedAuction.product.images[0].imageLink}" alt="Product Image" style="width:300px; height:auto;" />
                <a class="button" href="${process.env.FRONT_URL}"><span>${'Click here to visite the site'}</span></a>
              `;
    try {
  ...
  async getAllRegisteredUsers(batchSize: number = 1000) {
    ...
    try {
      ...
      console.log('Error while fetching user email address for bulk email:', error);
    }
  }
}
